...
export const FinanceStats = ({ stats }: Props) => {
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="card bg-base-100 border border-base-content/5 shadow-sm p-6 relative overflow-hidden group">
                ...
                <div className="relative z-10">
                    ...
                    <div className="text-4xl font-black tracking-tighter">{stats?.totalRevenueFormatted ?? "$... "}</div>
            ...
            <div className="card bg-primary text-primary-content shadow-lg shadow-primary/20 p-6 relative overflow-hidden">
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-xs font-black uppercase opacity-80 tracking-widest">Net Platform Revenue</span>
                    </div>
                    <div className="text-4xl font-black tracking-tighter">{stats?.netIncomeFormatted ?? "$..."}</div>
                    <div className="text-sm font-bold opacity-80 mt-1">
                        ~{stats ? (Number(stats.totalRevenue) > 0 ? ((Number(stats.totalRevenue.toString().replace(/[^0-9.-]+/g,"")) / 5) / Number(stats.totalRevenue.toString().replace(/[^0-9.-]+/g,"")) * 100).toFixed(0) : 0) : 20}% margin
                    </div>
                </div>
            </div>
            ...
            <div className="card bg-base-100 border border-base-content/5 shadow-sm p-6">
                ...
                <div className="text-4xl font-black tracking-tighter text-base-content/80">{stats?.pendingPayoutsFormatted ?? "$..."}</div>
                <div className="text-xs font-bold opacity-40 mt-1 uppercase">
                    Due to {stats?.activeInstructors ?? "multiple"} Instructors
                </div>
