...
import {
    ...
    Maximize2, Minimize2, Trash2, MessageSquare, Lock
} from "lucide-react";
...
export const TabSystem = ({ currentLesson, videoRef, onJumpToTime, isExpanded, onToggleExpand, isEnrolled }: TabSystemProps) => {
    ...
    const { data: savedNotes = [], isLoading: isNotesLoading } = useLessonNotes(currentLesson.id);
    const createNote = useCreateNote(currentLesson.id);
    const deleteNote = useDeleteNote(currentLesson.id);

    // Reset active tab and form when lesson changes
    useEffect(() => {
    ...
    // Live time tracker for the note input (only when video is available)
    useEffect(() => {
        if (activeTab !== "notes" || !videoRef) return;
        ...
        <div className="flex flex-col h-full bg-base-100">
            ...
            <div className="flex-1 overflow-y-auto relative custom-scrollbar bg-base-200/20">
                ...
                {activeTab === "discussion" && (
                    <div id="panel-discussion" role="tabpanel" className="h-full flex flex-col min-h-100 animate-in fade-in duration-300">
                        {!isEnrolled ? (
                            <div className="flex flex-col items-center justify-center h-full text-center px-4 opacity-70">
                                ...
                                <h3 className="text-lg font-bold text-base-content mb-2">Enroll to join the discussion</h3>
