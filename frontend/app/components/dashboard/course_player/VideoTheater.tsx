...
export const VideoTheater = ({
    ...
    }, [hasInteracted, togglePlay, skip, toggleFullscreen]);
        ...
        >
            ...
            {!hasInteracted && (
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
                    <button onClick={onPlayAction} className="group flex flex-col items-center gap-5 transition-transform hover:scale-105 active:scale-95" aria-label="Start lesson">
            ...
            {!isPlaying && hasInteracted && !isLoading && Math.abs(progress - 100) < 1 && (
                <div className="absolute inset-0 z-40 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md animate-in fade-in zoom-in-95 duration-300">
                    <div className="text-center p-8 rounded-3xl border border-white/10 bg-white/5 shadow-2xl max-w-md mx-4">
                        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-primary mb-3">Lesson Complete</p>
                        <h3 className="text-xl md:text-2xl font-bold text-white mb-8 line-clamp-2">{currentLesson.title}</h3>
                        <div className="flex flex-col sm:flex-row gap-3 justify-center">
                            <button onClick={() => { videoRef.current?.play(); }} className="btn btn-outline text-white border-white/20 hover:bg-white hover:text-black rounded-xl w-full sm:w-auto px-6" aria-label="Replay lesson">
                                <RotateCcw size={16} className="mr-2" /> Replay
                            </button>
                            <button onClick={onVideoEnd} className="btn btn-primary text-primary-content rounded-xl w-full sm:w-auto px-6 shadow-[0_0_20px_rgba(var(--p),0.4)]" aria-label="Go to next lesson">
            ...
            >
                ...
                <div className="relative w-full h-1.5 md:h-2 mb-4 group/progress flex items-center cursor-pointer">
                    ...
                    {/* Scrub Thumb — consistent size */}
                    <div
                        className="absolute h-3.5 w-3.5 bg-white rounded-full shadow-lg scale-0 group-hover/progress:scale-100 transition-transform duration-150 z-20 pointer-events-none"
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 md:gap-5">
                        {/* Play/Pause */}
                        <button onClick={togglePlay} className="text-white hover:text-primary transition-colors p-2 hover:bg-white/10 rounded-full" aria-label={isPlaying ? "Pause" : "Play"}>
                        ...
                        <div className="flex items-center gap-1 md:gap-2 text-white/80">
                            <button onClick={() => skip(-10)} className="hover:text-white p-2 hover:bg-white/10 rounded-full transition-colors" aria-label="Skip backward 10 seconds">
                                <RotateCcw size={18} />
                            </button>
                            <button onClick={() => skip(10)} className="hover:text-white p-2 hover:bg-white/10 rounded-full transition-colors" aria-label="Skip forward 10 seconds">
                                <RotateCw size={18} />
                            </button>
                        </div>
                        ...
                        <div className="flex items-center gap-2 group/volume relative">
                            <button onClick={() => setIsMuted(!isMuted)} className="text-white/80 hover:text-white p-2 hover:bg-white/10 rounded-full transition-colors" aria-label={isMuted ? "Unmute" : "Mute"}>
                                <VolumeIcon size={20} />
                            </button>
                    ...
                    <div className="flex items-center gap-2 md:gap-4">
                        {/* Next Lesson Button */}
                        <button onClick={onVideoEnd} className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider text-white/70 hover:text-primary hover:bg-white/10 py-2 px-3 rounded-lg transition-all" aria-label="Skip to next lesson">
                            <span className="hidden sm:inline">Next Lesson</span> <SkipForward size={16} />
                        </button>

                        {/* Fullscreen Toggle */}
                        <button onClick={toggleFullscreen} className="text-white/80 hover:text-white p-2 hover:bg-white/10 rounded-full transition-colors" aria-label={isFullscreen ? "Exit fullscreen" : "Enter fullscreen"}>
